"""Run script to train the MST parser implementation."""
...
def _pre_encode(h, xs, accumulate_vocab=False):
    if accumulate_vocab:
...
def evaluate(gold_file, data, name):
    ...
    uni_eval.write_predictions_to_file(predictions, reference_file=gold_file, output_file=output_file, vocab=vocab)
...
def train(training_data, dev_data_file, dev_data, epochs, model_param_file):
...
if __name__ == "__main__":
    ...
    s = time.time()
    TRAIN = _pre_encode(encoder, TRAIN, accumulate_vocab=True)
    DEV = _pre_encode(encoder, DEV)
    TEST = _pre_encode(encoder, TEST)
    ...
    # train model
    train(TRAIN, arguments.dev, DEV, arguments.epochs, arguments.model)

    # populate with best parameters
    parser.W = np.load("%s.npy" % arguments.model)

    print(">> Time to evaluate on test set")
    predictions, test_metrics = evaluate(arguments.test, TEST, "test")
    print(test_metrics)
