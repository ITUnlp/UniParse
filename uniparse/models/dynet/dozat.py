...
def biLSTM(builders, inputs, batch_size, dropout_x=0., dropout_h=0.):
    ...
    def run(self, word_inputs, tag_inputs, arc_targets=None, rel_targets=None):
        ...
        if self.pret_word_embs:
            word_embs = [
                ...
                dy.lookup_batch(self.pret_word_embs, w, update=False) for w in word_inputs
            ]
        else:
            word_embs = [
                dy.lookup_batch(self.word_embs, np.where(w < self._vocab.words_in_train, w, self._vocab.UNK))
        ...
        if is_train:
            ...
            emb_inputs = [
                dy.concatenate([dy.cmult(w, wm), dy.cmult(pos, posm)])
        ...
        top_recur = biLSTM(self.LSTM_builders, emb_inputs, dropout_input_p, dropout_hidden_p, batch_size)
        ...
        W_dep = self.mlp_dep_W
        b_dep = self.mlp_dep_b
        ...
        W_arc = self.arc_W
        arc_logits = bilinear(
            dep_arc, W_arc, head_arc, self.mlp_arc_size, n, batch_size, num_outputs=1, bias_x=True, bias_y=False)
        ...
        rel_probs = np.transpose(
            np.reshape(
                dy.softmax(dy.transpose(flat_rel_logits)).npvalue(), (self._vocab.rel_size, n, n, batch_size), 'F'))
        ...
        if is_train:
            ...
            return arc_predictions, rel_predictions, None
