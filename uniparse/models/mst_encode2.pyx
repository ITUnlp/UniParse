...
ctypedef vector[np.uint64_t] uint_vec
...
cdef class BetaEncodeHandler:
    def __init__(self):
        # cdef int_vec W = int_vec(5_000_000)

        cdef int d = 50
        cdef int n = 10
        cdef vector[vector[int]] v
        cdef vector[vector[vector[int]]] m3
        v.resize(n, vector[int](d, 0))
        m3.resize(n, v)
    ...
    def __call__(self, np.uint64_t[:] forms, np.uint64_t[:] upos, np.int64_t[:] target_arcs=None, np.int64_t[:] target_rels=None):
        ...
        cdef uint_vec pred_t = uint_vec(n)
        cdef uint_vec succ_t = uint_vec(n)

        cdef vector[vector[vector[np.uint64_t]]] yoyo;
